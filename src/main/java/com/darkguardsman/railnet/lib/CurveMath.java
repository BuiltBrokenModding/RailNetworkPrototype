...
public class CurveMath {
	...
	/**
	 * Creates curves between 2 points with smoothing based on the input location
	 * and direction
	 *
	 * @param start
	 *            Starting point
	 * @param startAngle
	 *            Starting points entering direction
	 * @param end
	 *            Ending point
	 * @param endAngle
	 *            Ending points leaving direction
	 * @param segmentCount
	 *            The number of segments to divide this rail into, minimum is 1, 2
	 *            would mean 3 points (beginning, middle, end) (more is smoother for
	 *            curves)
	 * @return
	 * @throws Exception
	 */
	...
	private float getCurveValue(double p1, double pt1, double p2, double pt2, double t) {
		return (float) (Math.pow(1 - t, 3) * p1 + 3 * Math.pow(1 - t, 2) * t * pt1 + 3 * (1 - t) * Math.pow(t, 2) * pt2
				+ Math.pow(t, 3) * p2);
	}
}